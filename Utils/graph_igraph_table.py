...
def generate_graph(features, clusters, t_drop=0.7, t_modify=0.8, max_depth=2):
    ...
    state_to_id = {}
    depth = {}  # Dictionary to keep track of node depths
    ...
    depth[source_id] = 0  # Source node has depth 0

    queue = [(source_node, 0)]  # Added depth to the queue elements
    while queue:
...
# def generate_graph(features, clusters, t_drop=0.7, t_modify=0.8, max_depth=1):
#     graph = Graph(directed=True)
#     state_to_id = {}
#     depth = {}  # Dictionary to keep track of node depths
#
...
#         if sum(items_status) > t_drop * features and current_depth < max_depth:
#             # "modify": try dropping each value of the state, remain at least ceil(t_modify * k) values
#             for j in range(1, clusters - math.ceil(t_modify * clusters) + 1):
#                 for subset in combinations(range(clusters), j):
#                     new_values_status = list(values_status)
#                     for value in subset:
